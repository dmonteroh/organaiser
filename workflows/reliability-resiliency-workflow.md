...
- Maximum Completion Self-Check attempts: 2 (manifest authority: `manifests/reliability-resiliency-workflow.v1.yaml`'s `caps.finalizeAttempts`). This is a separate cap from the revision-round cap above: a Completion Self-Check failure is not a step-7 challenger round, since it is discovered after step 9, not during step 7. Each Completion Self-Check failure that re-dispatches the investigator (Completion Self-Check, below) counts as one attempt. At cap exhaustion, stop re-dispatching and mark the assessment `waiting-operator` with the unresolved self-check items.
